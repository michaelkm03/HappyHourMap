...
cat "$INPUT_CSV" | while read -r NAME; do 
    ...
    while [ $REQUEST_ATTEMPTS -lt $MAX_ATTEMPTS ]; do
        ...
        if [ -n "$URL" ]; then
            ...
            # 4-COLUMN OUTPUT: key, url, address, raw_response
            ...
            echo "Response was: $RESPONSE" >> "$LOG_FILE"
    ...
    # Default delay between requests (runs only if request was successful or failed non-429)
    sleep $DEFAULT_DELAY 
